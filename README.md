...
- 手动添加账号
- 自动解析邮箱和账号信息
- Plan Type 选择（Free/Plus/Pro/Team/Enterprise）
- 批量删除账号
- 查看账号额度和使用情况
- 刷新账号额度（基于 ID Token 被动验证）
- 负载均衡策略配置
- 账号状态监控（active/error/payment_required/forbidden/rate_limited）

### 数据分析
- 请求量趋势图表（支持时间范围过滤）
- 模型使用分布统计
- 账号详细统计（请求数、成功率、响应时间、Token 使用量）
- Token 使用量追踪（输入/输出/总计）
- 费用估算（按模型统计 Top 5）
- API 请求日志（实时响应时间）
...
**新功能**
- ✅ 3小时额度自动重置机制（模拟 ChatGPT 行为）
- ✅ Token 使用量统计（输入/输出/总计 tokens）
- ✅ 费用估算功能（按模型统计 Top 5）
- ✅ 时间范围过滤（24小时/7天/30天/全部）
- ✅ 自动日志清理（可配置保留天数，默认 90 天）
- ✅ 本地时区支持（GMT+8）
- ✅ 账号状态追踪（active/error/payment_required/forbidden/rate_limited）
- ✅ 自动禁用失效账号（连续失败 5 次）
- ✅ Plan Type 手动选择（Free/Plus/Pro/Team/Enterprise）
- ✅ 自动解析邮箱和账号 ID（从 Access Token）
- ✅ 负载均衡策略数据库持久化

**改进**
- 🔧 所有统计数据支持时间范围过滤
- 🔧 从 ID Token 被动验证额度（避免触发风控）
- 🔧 实时响应时间追踪
- 🔧 账号详细统计新增 Token 使用量列
- 🔧 修复时区显示问题（UTC → GMT+8）
- 🔧 清理日志时自动重置账号统计（保留额度信息）
